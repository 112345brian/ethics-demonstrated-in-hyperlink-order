...
def matrix_html(graph: NodeGraph, current_node: dict, deps: list[dict], uses: list[dict]) -> str:
...
def write_node_pages(config: BuildConfig, graph: NodeGraph) -> None:
    ...
    for node in graph.ordered:
        ...
        prev_link = (
            f'<a href="{node_page_href(previous_node)}">Previous: {html.escape(previous_node["code"])}</a>'
            if previous_node else ""
        )
        next_link = (
            f'<a href="{node_page_href(next_node)}">Next: {html.escape(next_node["code"])}</a>'
            if next_node else ""
        )
        page = render_template(
            ...
            source_href=html.escape(node["href"]),
            source_query=quote(node["href"], safe=""),
            page_href=node_page_href(node),
            ...
            prev_link=prev_link,
            next_link=next_link,
            uses=node_link_list(deps, "No explicit dependencies recorded for this node."),
            used_by=node_link_list(uses, "No recorded uses of this node."),
            ancestors=chain_link_list(
                graph.transitive_items(href, "out"), "No transitive ancestors recorded.", node["code"], "out"
            ),
            descendants=chain_link_list(
                graph.transitive_items(href, "in"), "No transitive descendants recorded.", node["code"], "in"
            ),
            matrix=matrix_html(graph, node, deps, uses),
            graph=graph_html(node, deps, uses),
        )
