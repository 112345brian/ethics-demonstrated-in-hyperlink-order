...
def node_link_list(config: BuildConfig, items: list[dict], empty_text: str) -> str:
    if not items:
    ...
    for item in items:
        ...
        seen.add(key)
        rows.append(
            f'<li><a href="{html.escape(site_url(config.base_path, href))}">{html.escape(label)}</a>'
...
def chain_link_list(
    config: BuildConfig, items: list[dict], empty_text: str, root_code: str, direction: str
) -> str:
    if not items:
        return f"<p>{html.escape(empty_text)}</p>"
    ...
    for item in items[:160]:
        ...
        rows.append(
            ...
            f'<a href="{html.escape(site_url(config.base_path, item["href"]))}">'
            f'{html.escape(item.get("code") or item["text"])}</a>'
...
def render_node_source_html(config: BuildConfig, node: dict) -> str:
    ...
    for el in target.iter():
        ...
        if href:
            el.set("href", site_url(config.base_path, absolutize_href(node["file"], href)))
        src = el.attrib.get("src")
        if src and not re.match(r"^[a-z]+:", src) and not src.startswith("/"):
            el.set("src", site_url(config.base_path, "/" + (Path(node["file"]).parent / src).as_posix()))
...
def render_anchor_context_html(config: BuildConfig, href: str) -> str:
    ...
    for el in target.iter():
        ...
        if link:
            el.set("href", site_url(config.base_path, absolutize_href(path, link)))
        classes = set(el.attrib.get("class", "").split())
        if "cite" in classes or "gloss" in classes:
            classes.add("reference-link")
            el.set("class", " ".join(sorted(classes)))
...
def resource_cards(config: BuildConfig, items: list[dict], empty_text: str) -> str:
    ...
    for item in items[:40]:
        ...
        cards.append(
            f'<article class="resource-card"><h3><a href="{html.escape(site_url(config.base_path, href))}">'
